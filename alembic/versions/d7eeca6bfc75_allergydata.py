...
Revision ID: d7eeca6bfc75
Revises: 9b59b0aaae0c
Create Date: 2025-11-19 18:01:59.984380
...
revision: str = 'd7eeca6bfc75'
down_revision: Union[str, Sequence[str], None] = '9b59b0aaae0c'
...
def upgrade() -> None:
    """데이터 마이그레이션 실행: 100가지 알레르기 데이터를 'allergies' 테이블에 SQL로 직접 삽입합니다."""
    
    # 삽입할 데이터의 튜플 리스트를 준비합니다.
    # PostgreSQL의 INSERT 문법에 맞게 '알레르기 이름'만 전달합니다.
    value_strings = ', '.join([f"('{name.replace("'", "''")}')" for name in INITIAL_ALLERGIES])
    
    # 1. INSERT SQL 쿼리 생성
    # "id" 컬럼을 생략하고 "name" 컬럼에만 값을 넣어 DB가 id를 자동으로 할당하게 합니다.
    insert_sql = f"""
