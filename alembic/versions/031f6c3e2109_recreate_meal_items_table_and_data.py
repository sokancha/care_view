"""recreate meal_items table and data

Revision ID: 031f6c3e2109
Revises: cae82a649ed6
Create Date: 2025-11-23 21:39:39.275625

"""
import sqlalchemy as sa
from alembic import op
from typing import Sequence, Union
# SQLAlchemy의 테이블 및 컬럼 정의를 위해 필요한 모듈을 추가합니다.
from sqlalchemy.sql import table, column 
from sqlalchemy import String, Integer, Text 


# revision identifiers, used by Alembic.
revision: str = '031f6c3e2109'
down_revision: Union[str, Sequence[str], None] = 'cae82a649ed6'
branch_labels: Union[str, Sequence[str], None] = None
depends_on: Union[str, Sequence[str], None] = None


# 레시피에서 추출된 Meal Item 데이터 목록 (총 47개) - 사용자님 제공 데이터
INITIAL_MEAL_ITEMS = [
...
def upgrade() -> None:
    ...
    """
    # 1. Meal_Item 테이블 생성 (이전에 DROP 했으므로 다시 생성합니다)
    op.create_table(
        'meal_items',
        ...
        # 스키마는 기본 public을 가정하고 생략
    )
    
    # 2. bulk_insert를 위한 테이블 객체 정의: 데이터에 있는 컬럼만 명시
    # INITIAL_MEAL_ITEMS에는 item_id와 name만 있으므로 두 컬럼만 지정합니다.
...
def downgrade() -> None:
    ...
    # CASCADE 옵션을 사용하여 meal_items에 종속된 모든 객체(다른 테이블의 외래 키 등)도 함께 삭제합니다.
